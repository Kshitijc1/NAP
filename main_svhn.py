...
from helper import train_data_svhn, val_data_svhn
...
import wandb
wandb.init(project="SMART_svhn_2", name=args.name)
...
train_loader = torch.utils.data.DataLoader(train_data_svhn, batch_size=128, shuffle=True)
val_loader = torch.utils.data.DataLoader(val_data_svhn, batch_size=128, shuffle=False)
...
def main():
    global best_test
    model = WideResNet(depth=28, num_classes=num_class, widen_factor=2, dropRate=0.0)
    model.cuda()
    if args.load:
...
def train(train_loader, model, optimizer, epoch):
    ...
    for i, (xB, yB) in enumerate(train_loader):
            ...
            if args.loss == "trades":
                xB_adv = pgd_attackLinf(model, xB, yB, eps=.01, alpha=.0025, nSteps=10, rand=True, grad_func=grad_trades)
                ...
                xB_adv = pgd_attackLinf(model, xB, yB, eps=.01, alpha=.0025, nSteps=10, rand=True)
...
def validate(val_loader, model, epoch):
    ...
    for i, (xB, yB) in enumerate(val_loader):
            ...
            xB_adv = pgd_attackLinf(model, xB, yB, eps=.01, alpha=.001, nSteps=20, rand=False)
            with torch.no_grad():
    ...
    if best_test < top1_a.avg:
        ...
        gsave(model.state_dict(), "gs://vyasnikhil96/models/SMART/wandb_%s/" % (wandb.run.id)+str(epoch))
